...
switch ($action) {
    case 'approve_verification':
        ...
        try {
            ...
            // 3. Send congratulatory system notification
            ...
            $notifStmt->execute([
                'id' => $notifId,
                'user_id' => $userId,
                'message' => "Your quest completion was verified! You earned +{$xpReward} XP and +{$coinsReward} Gold Coins.",
            ]);
        ...
        if (!empty($completionId)) {
            try {
                $stmt = $db->prepare("
                    ...
                    SET status = 'review',
                        reviewed_by = :admin,
                        review_notes = 'Flagged for secondary evidence review'
                    WHERE id = :id
                ");
                $stmt->execute([
                    'admin' => $adminId,
