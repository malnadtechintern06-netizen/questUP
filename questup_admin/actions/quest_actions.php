<?php
/**
 * QuestUP Admin - Quest Operations Action Handler
 */
...
switch ($action) {
    case 'create_quest':
        ...
        $verSecret = trim($_POST['verification_secret'] ?? '') ?: null;
        $quizJson = trim($_POST['quiz_data_json'] ?? '') ?: null;
        $minDuration = max(0, (int)($_POST['min_duration_seconds'] ?? $reqDuration));
        $adminReview = isset($_POST['requires_admin_review']) ? 1 : 0;
        $allowGallery = isset($_POST['allow_gallery_upload']) ? 1 : 0;
        ...
        try {
            $stmt = $db->prepare("
                INSERT INTO quests (
                    ...
                    verification_secret, quiz_data_json, min_duration_seconds,
                    requires_admin_review, allow_gallery_upload,
                    created_at
                ) VALUES (
                    :id, :title, :description, :storyline, :category, :verification_type,
                    :latitude, :longitude, :radius_meters, :xp_reward, :coins_reward,
            ...
            $stmt->execute([
                ...
                'icon_key' => $iconKey,
                'verification_secret' => $verSecret,
                'quiz_data_json' => $quizJson,
        ...
        try {
            $stmt = $db->prepare("
                ...
                SET title = :title,
                    ...
                    allow_gallery_upload = :allow_gallery_upload
                WHERE id = :id
            ");
            $stmt->execute([
                'title' => $title,
            ...
            try {
                ...
                if ($src) {
                    ...
                    $insert->execute([
                        ...
                        'verification_secret' => $src['verification_secret'] ?? null,
                        'quiz_data_json' => $src['quiz_data_json'] ?? null,
                        'min_duration_seconds' => (int)($src['min_duration_seconds'] ?? 0),
                        'requires_admin_review' => (int)($src['requires_admin_review'] ?? 0),
                        'allow_gallery_upload' => (int)($src['allow_gallery_upload'] ?? 0),
                    ]);
    ...
    case 'delete_quest':
        ...
        if (!empty($questId)) {
            try {
                ...
                $stmt = $db->prepare("DELETE FROM quest_verification_attempts WHERE quest_id = :id");
                $stmt->execute(['id' => $questId]);
                ...
                set_flash('success', 'Quest and related records deleted successfully.');
            } catch (PDOException $e) {
                if ($db->inTransaction()) {
                    $db->rollBack();
                }
